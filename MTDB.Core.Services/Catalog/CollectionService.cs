...
namespace MTDB.Core.Services.Catalog
{
    public class CollectionService
    {
        ...
        {
            var teams = await _dbContext.Set<Team>()
        ...
        public async Task<CollectionDetails> GetPlayersForCollection(int skip, int take, string sortByColumn, SortOrder sortOrder, string groupName, string name, CancellationToken token, bool showHidden = false)
        {
            ...
            IQueryable<Player> query = _dbContext.Set<Player>()
                .Include(p => p.Tier);
            ...
            {
                var team = await _dbContext.Set<Team>().FirstOrDefaultAsync(p => p.Name == name, token);
                if (team == null)
                    return null;
            ...
            {
                var collection = await _dbContext.Set<Collection>().FirstOrDefaultAsync(p => (p.GroupName == groupName || p.ThemeName == groupName) && p.Name == name, token);
        ...
        {
            var entity = await _dbContext.Set<Collection>()
