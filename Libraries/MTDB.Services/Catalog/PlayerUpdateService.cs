...
namespace MTDB.Core.Services.Catalog
{
    public class PlayerUpdateService
    {
        ...
        private async Task<bool> UpdatePlayersFromFile(string path, CancellationToken token)
        {
            ...
            var update = await _dbContext.Set<PlayerUpdate>()
            ...
            var badges = await _dbContext.Set<Badge>()
                .ToListAsync(token);
            var tendencies = await _dbContext.Set<Tendency>()
                .ToListAsync(token);

            var ids = list.Keys.ToArray();
            var existingIds = await _dbContext.Set<Player>()
            ...
            foreach (var filePlayer in list)
            {
                ...
                {
                    _dbContext.Set<PlayerUpdateChange>().Remove(changeToRemove);
                }
            }
            ...
            if (update.Changes.Any())
            {
                ...
                if (isNew)
                    _dbContext.Set<PlayerUpdate>().Add(update);
        ...
        public async Task<Paged<PlayerUpdatesViewModel>> GetUpdates(int skip, int take, CancellationToken token)
        {
            ...
            var wmsquery = _dbContext.SqlQuery<PlayerUpdatesViewModel>(string.Format(request,
                string.Empty,
            ...
            var countQ = _dbContext.SqlQuery<int>(string.Format(request,
                "select count(*) from (",
                "dist.CreatedDate",
                ") As Z"));
        ...
        public async Task<PlayerUpdateDetailsModel> GetUpdate(DateTimeOffset date, int skip, int take, CancellationToken token)
        {
            ...
            var changesQuery = _dbContext.SqlQuery<ChangeRow>(changesRequest,
                new SqlParameter("skip", skip),
                new SqlParameter("take", take),
                new SqlParameter("date", date));
            var countQuery = _dbContext.SqlQuery<int>(countRequest,
                new SqlParameter("date", date));
        ...
        public async Task<List<PlayerUpdateViewModel>> GetAllNewCards(DateTimeOffset date, CancellationToken token)
        {
            ...
            var newCardsQuery = _dbContext.SqlQuery<ChangeRow>(newCardsRequest,
                    new SqlParameter("date", date));
        ...
        {
            var update = await _dbContext.Set<PlayerUpdate>().FilterByCreatedDate(dateTime).FirstOrDefaultAsync(token);
        ...
        public async Task<bool> PublishUpdate(DateTime date, string title, CancellationToken token)
        {
            ...
            var badges = await _dbContext.Set<Badge>().ToListAsync(token);
            var tendencies = await _dbContext.Set<Tendency>().ToListAsync(token);

            //performance optimization! Be carefully 
            var playerIds = await _dbContext.Set<PlayerUpdateChange>()
            ...
            {
                //performance optimization! Be carefully
                var playerChanges = await _dbContext.Set<PlayerUpdateChange>()
                    .Where(puc => puc.PlayerId == playerId && puc.PlayerUpdateId == update.Id)
                    .ToListAsync(token);

                //performance optimization! Be carefully
                var query = _dbContext.Set<Player>().AsQueryable();
                ...
                var aggregated = await player.AggregateStats(_statService, token);
        ...
        public async Task<bool> DeleteUpdate(DateTime date, CancellationToken token)
        {
            ...
            _dbContext.Set<PlayerUpdateChange>().RemoveRange(update.Changes);
            _dbContext.Set<PlayerUpdate>().Remove(update);
            await _dbContext.SaveChangesAsync(token);
        ...
        public async Task DetermineChanges(Player oldPlayer, Player player, CancellationToken token)
        {
            ...
            if (changes.Any())
            {
                if (existingUpdates == null)
                {
                    ...
                    _dbContext.Set<PlayerUpdate>().Add(dbUpdate);
                }
                else
                {
